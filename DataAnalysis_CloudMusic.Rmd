...
```{r}
hc[hc[,"userid"]==(hc_regular[hc_regular[,"count"]==10,]$userid),c(3,4,6)]
```
...
hot_comment_freq <- ggplot(data = hc_all, aes(x=count, fill=用户类型, color=用户类型)) + geom_histogram(binwidth = 1) + labs(x = "上榜次数", y = "人数") + ggtitle("人数随上榜次数的分布") + theme(plot.title = element_text(hjust = 0.5)) + xlim(0,6)
hot_comment_freq
```
...
length_freq <- ggplot(data = all_length, aes(x=评论字数, fill=评论类型, color=评论类型)) + geom_histogram(aes(y=..density..),binwidth = 5) + labs(x = "评论字数", y = "频率") + ggtitle("评论的字数出现频率") + theme(plot.title = element_text(hjust = 0.5)) + xlim(0,150) + scale_fill_brewer(palette = "Pastel1")
length_freq
```
...
```{r warning=FALSE}
length_freq_year <- ggplot(data = hc_length_year, aes(x=评论字数,fill=评论年份,colour=评论年份)) + geom_density(alpha=0.2) + labs(x = "评论字数", y = "频率") + ggtitle("2014-2021评论的字数出现频率") + theme(plot.title = element_text(hjust = 0.5)) + xlim(0,150)
length_freq_year
```
